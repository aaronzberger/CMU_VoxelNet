...
def cal_iou2d(box1_corner, box2_corner):
    config = load_config()
    voxel_W, voxel_H, voxel_D = get_num_voxels()
    box1_corner = np.reshape(box1_corner, [4, 2])
    box2_corner = np.reshape(box2_corner, [4, 2])
    box1_corner = (
        (voxel_W, voxel_H)
        - (box1_corner
           - (config['pcl_range']['X1'], config['pcl_range']['Y1']))
        / (config['voxel_size']['W'], config['voxel_size']['H'])
    ).astype(np.int32)
    box2_corner = (
        (voxel_W, voxel_H)
        - (box2_corner
    ...
    buf1 = np.zeros((voxel_H, voxel_W, 3))
    buf2 = np.zeros((voxel_H, voxel_W, 3))
        ...
        for idx in range(len(gt_box3d_corner)):
            ...
            while is_collision and _count < 100:
                ...
                for idy in range(idx):
                    iou = cal_iou2d(
                        tmp[0, :4, :2], gt_box3d_corner[idy, :4, :2])
                    if iou > 0:
                        is_collision = True
                        _count += 1
                        break
